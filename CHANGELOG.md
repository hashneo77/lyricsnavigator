...
### Fixed
- Users who left a session were still controlled by the session creator
- Firebase onValue listeners for song sync and participant count are now properly detached on leave/end
- Extracted shared cleanup into detachSessionListeners() helper

## [1.0.8] - 2025-02-12

### Added
- Live participant count in session badge with people icon
- Firebase presence tracking per device
- Auto-cleanup on browser close/network loss via onDisconnect

## [1.0.7] - 2025-02-12

### Changed
- Session creator/joiner role system: creator can End (deletes session), joiners can only Leave
- Replaced session status bar with sleek inline badge with live dot indicator
- Create/Join controls hide when in an active session
- Removed session timer display

### Removed
- Session countdown timer UI

## [1.0.6] - 2025-02-11

### Removed
- Dark mode feature and toggle button
